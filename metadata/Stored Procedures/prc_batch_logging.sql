...
CREATE procedure [metadata].[prc_batch_logging] 
	...
	IF ((Select count(1) from metadata.audit_log where job_object_id in (Select job_object_id from metadata.job_object_details 
	 ...
	 Begin
	    if (isnull((select count(distinct job_object_id) FROM metadata.[audit_log] with(nolock) where batch_id=@paramSpBatchId and status='success'),0)  =
			   isnull((select count(distinct job_object_id) FROM metadata.job_object_details with(nolock) where source_id=@paramSpSourceId and enabled=1 and (job_id in ( 1,2,3,4,5,6) or (job_id=7 and target_object_name like 'dwh.recon_gl%') )),0))
	 ...
	 else if (@IsWeekEnd=1) --Check if weekend then do not consider job id 6
	 begin
	 if (isnull((select count(distinct job_object_id) FROM metadata.[audit_log] with(nolock) where batch_id=@paramSpBatchId and status='success'),0)  =
			   isnull((select count(distinct job_object_id) FROM metadata.job_object_details with(nolock) where source_id=@paramSpSourceId and job_id not in (6) and enabled=1),0))
			set @BatchRunStatus= 'Successfully Completed' 		 
		else
			set @BatchRunStatus= 'Partially Failed'	 
	 end
	 else --- Exclude Job id 6 (summarized) while checking success count
	  Begin
		if (isnull((select count(distinct job_object_id) FROM metadata.[audit_log] with(nolock) where batch_id=@paramSpBatchId and status='success'),0)  =
