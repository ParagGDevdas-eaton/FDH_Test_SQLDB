 ...
 --Oracle
--exec [metadata].[prc_email_details_Summ_recon] 383,'summarized','success',1,6
--exec [metadata].[prc_email_details_Summ_recon] 383,'summarized','failed',1,6
--exec [metadata].[prc_email_details_Summ_recon] 383,'recon','success',1,7	
--exec [metadata].[prc_email_details_Summ_recon] 383,'recon','failed',1,7
--SAP
--exec [metadata].[prc_email_details_Summ_recon] 169,'summarized','success',1,6
--exec [metadata].[prc_email_details_Summ_recon] 169,'summarized','failed',1,6
--exec [metadata].[prc_email_details_Summ_recon] 164,'recon','success',4,7	
--exec [metadata].[prc_email_details_Summ_recon] 164,'recon','failed',4,7

 
CREATE procedure bkp.[prc_email_details_summ_recon]
...
--- Recon : Abhiraj
 IF ( @paramSpJobId=7 )
	begin
		set @body = cast( (
		select td = table_name + '</td><td>' + cast( TimeTaken as varchar(30) ) + '</td><td>' 
		+ cast( source_row_count as varchar(30))  + '</td><td>' + cast( target_row_count as varchar(30)) + '</td><td>' + [status] +'</td><td>' + [rejeted_row_count]
		from (
			  select obj.table_name, 
				CAST((object_end_date-object_start_date) as time(0))  AS TimeTaken,
				a.[status] ,source_row_count=isnull(a.source_row_count,0),target_row_count=isnull(a.target_row_count,0),a.rejeted_row_count
			  from metadata.[audit_log] a with(nolock) inner join metadata.job_object_details o with(nolock) on a.job_object_id=o.job_object_id and a.source_id=o.source_id and a.job_id=o.job_id
				   inner join metadata.object obj on o.object_id=obj.object_id
			  where  batch_id=@paramSpBatchId and a.status=@paramSpStatus and o.source_id=@paramSpSourceId and o.job_id=@paramSpJobId and o.enabled=1 
			  ) as d
		...
		set @body = @bodyheader +'<table cellpadding=''2'' cellspacing=''2'' border=''1''>'
				  + '<tr><th>Table Name</th><th>TimeTaken[HH:MM:SS]</th><th>SourceRowCount</th><th>TargetRowCount</th><th>Status</th><th>ReconMismatchRecondCount</th></tr>'
				  + replace( replace( @body, '&lt;', '<' ), '&gt;', '>' )
				  + '</table>'
		If @totaltablecount = 0
			set @body ='Hi Team, <br/><br/> There is no success records for current execution <br/><br/>'	 	
...
IF (((@paramSpJobId=6) or (@paramSpJobId=7))  and ((Select count(1) from metadata.audit_log where job_object_id in (Select job_object_id from metadata.job_object_details 
